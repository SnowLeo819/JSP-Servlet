						...
						<c:forEach items="${boardList }" var="boardDto" varStatus="loop">
							<tr>
								...
								<td class="space${boardDto.reStep } left"
									...
									</c:if> <a href="../board/View.do?no=${boardDto.no }&num=${boardDto.num }">${boardDto.subject }</a>
								</td>
				...
				<div class="pagination">
					<ul>
						...
						<c:if test="${startPage != 1 }">
							<li><a
								href="../board/BoardList.do?clickPage=${startPage-pageBlock }&search_select=${param.search_select}&search_word=${param.search_word}"><span
									class="material-icons"> chevron_left </span></a></li>
						</c:if>
						<c:forEach var="page" begin="${startPage }" end="${endPage }"
							step="1" varStatus="loop">
							<li class=${clickPage==page?'active':'' }><a
								href="../board/BoardList.do?clickPage=${page }&search_select=${param.search_select}&search_word=${param.search_word}">${page }</a></li>
						</c:forEach>
						<c:if test="${endPage != lastPage }">
							<li><a
								href="../board/BoardList.do?clickPage=${startPage+pageBlock }"><span
									class="material-icons"> chevron_right </span></a></li>
						</c:if>
					</ul>
				</div>
				<form action="../board/BoardList.do" class="from" method="get">
					<div class="searchBox">

    ...
    <main>
      <div class="container">
		  ...
		  <div id="contents">
		  	...
		  	<form method="POST" action="join_process2.jsp" id="join" class="form">
		  		<table>
		  			<colgroup>
		  				<col style="width:20%">
		  				<col style="width:80%">
		  			...
		  			<tbody>
		  				<tr>
		  					...
		  					<td>
			  					<input type="text" name="user_id" id="user_id" placeholder="아이디를 입력하세요." style="width:80%">
			  					<input type="hidden" name="id_duplicate" value="">
			  					<button type="submit" class="btn Duplicate">중복검사</button>
		  					</td>
		  				</tr>
		  				<tr>
    ...
    <script>
    	$(".btnConfirm").on("click",function(){
    		if($("#user_id").val()=="") {
    			...
    			$("#user_id").focus();
    			return false;
    		} else if (<%=dupliTest.equals("firstTime")%>) {
    			alert("중복검사를 해주세요.")
    			$(".Duplicate").focus();
    			return false;
    		} else if (<%=dupliTest.equals("duplicate")%>) {
    			alert("중복된 아이디 입니다.")
    			$(".Duplicate").focus();
    			return false;
    		} else if($("#user_pw").val()=="") {
